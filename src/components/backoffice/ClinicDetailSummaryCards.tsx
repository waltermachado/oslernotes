  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="bg-dark-card border border-gray-800 rounded-2xl p-6">
        <div className="text-sm text-gray-400">Contas</div>
        <div className="mt-3 space-y-2">
          <div className="text-sm text-gray-200">Admins: {props.usersCount.admin}</div>
          <div className="text-sm text-gray-200">Médicos: {props.usersCount.medico}</div>
          <div className="text-sm text-gray-200">Atendentes: {props.usersCount.atendente}</div>
        </div>
      </div>

      <div className="bg-dark-card border border-gray-800 rounded-2xl p-6">
        <div className="text-sm text-gray-400">Plano & Limites</div>
        <div className="mt-3 space-y-2">
          <div className="text-sm text-gray-200">Plano atual: {props.planLabel(props.tier)}</div>
          {props.plan ? (
            ...
            <div className="text-xs text-gray-500">Sem catálogo aplicado</div>
          )}
        </div>
        <div className="mt-4 flex items-center gap-2">
          ...
          <button
            ...
            className="bg-brand-blue hover:bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors"
          >
            Aplicar
          </button>
        </div>
      ...
      <div className="bg-dark-card border border-gray-800 rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-400">Credenciais</div>
          <ShieldCheck className="w-4 h-4 text-gray-500" />
        </div>
        <div className="mt-3 space-y-2">
          <div className="text-xs text-gray-500">Cria conta e envia credencial para o responsável.</div>
          <div className="grid grid-cols-1 gap-2">
            <div className="relative">
              ...
              <input
                ...
                placeholder="E-mail"
                className="w-full bg-dark-input border border-gray-800 rounded-xl pl-9 pr-4 py-2 text-sm text-gray-200"
              />
            </div>
            <input
              value={credName}
            ...
            {credStatus && (
              <div className={cn('text-xs', credStatus.toLowerCase().includes('falha') ? 'text-red-400' : 'text-gray-400')}>
                {credStatus}
              </div>
            )}
          </div>
